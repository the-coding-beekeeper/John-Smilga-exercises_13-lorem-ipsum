...
btnEl.addEventListener("click", amountParagraphs);
...
function generateParagraphs() {
  ...
  createdParagraph.textContent = "";

  for (let i = 0; i < 50; i++) {
    let randomWord = words[Math.floor(Math.random() * words.length)];
    let word = i === 0 ? randomWord.charAt(0).toUpperCase() + randomWord.slice(1) : randomWord;
    createdParagraph.textContent += `${word} `;
  }
  createdParagraph.textContent = createdParagraph.textContent.trim() + ".";
  mainEl.appendChild(createdParagraph);
}
